      ...
      <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
            @foreach($laporanDumas as $laporan)
            <tr class="whitespace-nowrap">
              <td class="ps-8 lg:ps-6 xl:ps-6 pe-6 py-3">
                <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">
                    {{ $laporan->nama_peserta_diklat ?? $laporan->nama_peserta_pkl ?? $laporan->nama_pengguna_fasilitas ?? $laporan->nama_masyarakat_umum ?? 'N/A' }}
                </span>
              </td>            
                <td class="px-6 py-3">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">{{ $laporan->jenis_layanan ?? 'N/A' }}</span>
                </td>
                <td class="px-6 py-3">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">{{ $laporan->tipe ?? 'N/A' }}</span>
                </td>
                <td class="px-6 py-3">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">{{ $laporan->kategoriPengaduan->nama ?? 'N/A' }}</span>
                </td>
                <td class="px-6 py-3">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">
                        {{ $laporan->tanggal_pengaduan ? $laporan->tanggal_pengaduan->format('d F Y') : 'N/A' }}
                    </span>
                </td>
